...
namespace LSC1DatabaseLibrary.LSC1JobRepresentation.JobDataConverter
{
    public class JobDataToJobSturctureConverter
    {
        ...
        public LSC1StructuredJob<InstructionStep> Convert()
        {
            ...
            data.JobInfo = Job.JobName;
            var firstStep = new LSC1JobDataStep<InstructionStep>();
            firstStep.JobDataStepRow = CurrentJobData;
            ...
            while (!SimulationOver)
            {
                if(JobDataChanged)
                {
                    ...
                    currentJobDataStep.Frame = CurrentFrame;
                    currentJobDataStep.MoveParam = CurrentMoveParam;
                    currentJobDataStep.Tool = CurrentTool;
                    currentJobDataStep.JobDataStepRow = CurrentJobData;
